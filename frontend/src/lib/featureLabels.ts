export const FEATURE_LABELS: Record<string, string> = {
  ...
  drawdown_pct_504d:        'Pullback from recent highs',
  drawdown:                 'Pullback from recent highs',
  ret_20d:                  '20-day SPY return',
  momentum_20d:             '20-day price momentum',
  dist_sma50:               'Distance from 50-day moving average',
  emv_level:                'Equity volatility backdrop',
  emv_lag_20d:              'Equity volatility backdrop',
...
const NARRATIVE_FRAGMENTS: Record<string, { up: string; down: string }> = {
  ...
  drawdown:                 { up: 'a pullback from the 2-year high',                         down: 'proximity to the 2-year high' },
  ...
  emv_level:                { up: 'a firmer volatility backdrop',                      down: 'calmer day-to-day conditions' },
  emv_lag_20d:              { up: 'a firmer volatility backdrop in recent weeks',      down: 'calmer day-to-day conditions' },
...
export function narrativeFragmentFor(feature: string, direction: 'up' | 'down'): string {
  const entry = NARRATIVE_FRAGMENTS[feature]
  if (entry) return entry[direction]
  const label = FEATURE_LABELS[feature]
  if (label) return label.toLowerCase()
  return direction === 'up' ? 'elevated market conditions' : 'stable market conditions'
}
...
const SENTENCE_TEMPLATES: Record<string, { up: string; down: string }> = {
  ...
  drawdown:                 { up: 'SPY has pulled back from its 2-year high',                            down: 'SPY is near its 2-year high' },
  ...
  emv_level:                { up: 'The equity volatility backdrop is elevated',                      down: 'The equity volatility backdrop has been calm' },
  emv_lag_20d:              { up: 'The equity volatility backdrop has been elevated recently',        down: 'The equity volatility backdrop has been calmer recently' },
...
export function sentenceFor(feature: string, direction: 'up' | 'down'): string {
  const entry = SENTENCE_TEMPLATES[feature]
  if (entry) return entry[direction]
  const label = FEATURE_LABELS[feature]
  if (label) return direction === 'up' ? `${label} is elevated` : `${label} is subdued`
  return direction === 'up'
    ? 'An additional factor is contributing to elevated risk'
    : 'An additional factor is helping to limit risk'
}
